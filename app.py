...
def normalize_input_df(df: pd.DataFrame) -> pd.DataFrame:
    df2 = df.copy()
    # required/expected columns should exist in user's file
    ...
    # keep a monthly timestamp too (not used for daily trend, but handy)
...
if uploaded_file:
    ...
    df = df[(df["Date/Time Closed"] >= start_date) & (df["Date/Time Closed"] <= end_date)]
    if status_values:
